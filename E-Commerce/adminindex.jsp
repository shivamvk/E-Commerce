...
function addaproductfunction(event){
	...
	$.get("actionAddAProduct.jsp", $.param(params), function(response) {
...
function showeditproductmodal(productId){
	$('#editproductmodal').on('show.bs.modal', function() {
		...
		$.ajax({
            ...
            type: 'get',
            async: true,
            url: 'actionEditProductModal.jsp?productId=' + productId,
...
<div id="mySidenav" class="sidenav">
  ...
  <a href="">Dashboard</a>
  <a href="">Clients</a>
  <a href="actionAdminLogout.jsp">Logout</a>
</div>
		...
		productImagePath = productImagePath.replace("productImages/", "productImages%2F");
				%>
				  ...
				  <form  ENCTYPE="multipart/form-data" ACTION="fileupload.jsp?productId=<%=rs.getString(1) %>" METHOD=POST>
					       <input id="changeproductfilebutton<%=rs.getString(1) %>" onchange="uploadfileonfirebase(event,<%=rs.getString(1)%>)" style="display:none;" type="file" name="f1" required>
				  ...
				  <div class="card-footer">
				  	...
				  	<a href="actionAdminDeleteProduct.jsp?productId=<%=rs.getString(1) %>" class="btn btn-danger btn-sm"><span style="color:#ffffff">Delete</span></a>
				  </div>
				</div>
			</div>
			<%
...
<script>
  // Initialize Firebase
  var config = {
    apiKey: "AIzaSyDQ0oEOe5miCXKqOxjBXWj8p-sFlnZFsUQ",
    authDomain: "e-commerce-29be3.firebaseapp.com",
    databaseURL: "https://e-commerce-29be3.firebaseio.com",
    projectId: "e-commerce-29be3",
    storageBucket: "e-commerce-29be3.appspot.com",
    messagingSenderId: "919900666759"
  };
  firebase.initializeApp(config);
  function uploadfileonfirebase(e,id){
	...
	window.location.assign('actionUploadImage.jsp?image=' + downloadURL + '&id=' + id);
  });
  });
}
</script>
